    ...
    with col_left:
        ...
        with sub_tab1:
            ...
            if st.button("로그인", use_container_width=True):
                user_df = df[df['name'] == l_name] if not df.empty else pd.DataFrame()
                if not user_df.empty and str(user_df.iloc[0]['password']).strip() == l_pw:
                    st.session_state.logged_in = True
                    st.session_state.user_name = l_name
                    st.session_state.user_pw = l_pw
                    st.session_state.selected_dates = set(user_df[user_df['date'].notna()]['date'].unique())
                    st.rerun()
                else:
                    st.error("닉네임 또는 비번이 틀립니다.")
        ...
        for week in cal:
            ...
            for i, day in enumerate(week):
                if day == 0: w_cols[i].empty()
                    ...
                    w_cols[i].button(btn_label, key=f"st_{date_str}", help=nicknames, use_container_width=True, disabled=True, type="primary" if is_best_day else "secondary")

    # [하단 영역: 내 일정 수정 - 요일 포함 달력 디자인]
    if st.session_state.logged_in:
        st.divider()
        st.subheader(f"📍 {st.session_state.user_name}님의 일정 수정")
        st.info("가능한 날짜를 클릭하여 선택하세요.")
        ...
        for week in cal_in:
            ...
            for i, day in enumerate(week):
                if day == 0: w_cols_in[i].empty()
                    ...
                    if w_cols_in[i].button(str(day), key=f"sel_{date_str}", use_container_width=True, type="primary" if is_sel else "secondary"):
        ...
        if st.button("💾 내 일정 최종 저장", use_container_width=True):
            save_schedule(DB_PATH, st.session_state.user_name, st.session_state.user_pw, list(st.session_state.selected_dates), {})
            st.success("저장되었습니다!")
            st.rerun()
